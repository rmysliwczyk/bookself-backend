...
from app.db_operations.book import (
    BookNotFound,
    create_book,
    read_book,
    update_book,
...
def test_book_create_model_validation_fails_with_title_longer_than_max_length():
    ...
    with pytest.raises(ValidationError) as exc:
        BookCreate(
            title=title, rating=10, visibility_to_others=True, user_id=fake_user_id
        )
